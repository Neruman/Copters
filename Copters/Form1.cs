...
namespace Copters
{
    ...
    public partial class Form1 : Form
    {
        ...
        public List<RadialCoodrs> BadPointHookUps = new List<RadialCoodrs>();
        public List<RadialCoodrs> BadPointCenters = new List<RadialCoodrs>();
        public List<RadialCoodrs> FastMemoryHookUps = new List<RadialCoodrs>();
        public List<RadialCoodrs> FastMemoryCenters = new List<RadialCoodrs>();
        ...
        public void RotateCopter(RadialCoodrs CopterCenter, double CopterRad)
        {
            ...
            for (double CopterRotateAngle = 0; CopterRotateAngle < 120; CopterRotateAngle += 0.5d)
            {
                ...
                {
                    if (!BadPointCenters.Contains(CopterCenter)) { BadPointCenters.Add(CopterCenter); FastMemoryCenters.Add(CopterCenter); }
                    CopterCenter.SetHookUps(CopterRadius, CopterRotateAngle);
                    if (!BadPointHookUps.Contains(CopterCenter.GetHookUp1)) { BadPointHookUps.Add(CopterCenter.GetHookUp1); FastMemoryHookUps.Add(CopterCenter.GetHookUp1); }
                    if (!BadPointHookUps.Contains(CopterCenter.GetHookUp2)) { BadPointHookUps.Add(CopterCenter.GetHookUp2); FastMemoryHookUps.Add(CopterCenter.GetHookUp2); }
                    if (!BadPointHookUps.Contains(CopterCenter.GetHookUp3)) { BadPointHookUps.Add(CopterCenter.GetHookUp3); FastMemoryHookUps.Add(CopterCenter.GetHookUp3); }
                    // Обратить внимание
        ...
        public void Experiment()
        {
            ...
            {
                numericUpDown4.Value = (Decimal)(CopterShift);
                numericUpDown4.Refresh();

                for (double CopterPhi = 0; CopterPhi < 360; CopterPhi = CopterPhi + 0.5d)
                {
        ...
        {
            foreach (RadialCoodrs HookUp in FastMemoryHookUps)
            {
                PrintPoint(HookUp);
            }
        ...
        {
            foreach (RadialCoodrs center in FastMemoryCenters)
            {
                PrintCenter(center);
                //PrintCenter(center.Radius, center.phi);
            }
            FastMemoryCenters.Clear();
        }
        /*
        public void LabelRefresh(string Message)
        {
            //label6.Text = Message;
        ...
        */
        ...
        {
            CopterAngle = (double)numericUpDown3.Value;
        }
        private void numericUpDown4_ValueChanged(object sender, EventArgs e)
        {
            CopterR = (double)numericUpDown4.Value;
        ...
        {
            CopterPhi = (double)numericUpDown5.Value;
        }
